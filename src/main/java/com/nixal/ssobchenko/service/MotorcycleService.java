...
import com.nixal.ssobchenko.repository.HibernateMotorcyclesRepository;
...
public class MotorcycleService extends VehicleService<Motorcycle> {
    ...
    private MotorcycleService(HibernateMotorcyclesRepository repository) {
        super(repository);
    }

    public static MotorcycleService getInstance() {
        if (instance == null) {
            instance = new MotorcycleService(HibernateMotorcyclesRepository.getInstance());
        }
        return instance;
    }
